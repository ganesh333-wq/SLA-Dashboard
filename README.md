## 📊 SLA Dashboard Preview

![SLA Dashboard](SLA dashboard/updated%20sla%20dashboard%20img.jpg)









# 📊 SLA Dashboard – Power BI Project

This project contains an interactive Service Level Agreement (SLA) Dashboard built using Microsoft Power BI. The dashboard provides a visual overview of SLA performance, enabling users to monitor service efficiency, identify delays, and track compliance over time.

# 🔍 Key Features

 SLA Compliance Tracking: Visual indicators for tickets meeting or breaching SLA targets.

 Performance Metrics: Aggregated KPIs including resolution times, response times, and compliance percentages.

 Trend Analysis: Time-based visuals to analyze SLA performance patterns.

 Interactive Filtering: Multi-level slicers allowing users to drill down by date, category, priority, or team.

 User-Friendly Layout: Clean, intuitive visuals optimized for quick decision-making.

# 📁 File Included

SLA dashboard.pbix – The full Power BI report file.
Dataset file - Cleaned file ready to use

# 🚀 How to Use

Open the .pbix file in Power BI Desktop.

Refresh or connect to your data source if needed.

Explore the visuals and use the slicers to interact with the report.
